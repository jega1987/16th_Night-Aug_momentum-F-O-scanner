...
Three things Kite does better than the previous broker, and this module uses
all three:
...
* **`quote()` returns depth**, so option bid/ask spread is available - which is
  what the liquidity gate needs and 5paisa could not supply.
...
class KiteFeed(MarketFeed):
    ...
    def __init__(self):
        ...
        # Kite's documented ceilings are tighter than the previous broker's:
        # roughly 3/s for historical and 1/s for quote. One conservative
        # limiter covers both.
...
class KiteTickStream:
    ...
    reconnection internally, so unlike the previous broker there is no
    hand-rolled run_forever loop here.
    """
