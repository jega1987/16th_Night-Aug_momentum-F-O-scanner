...
async def main() -> int:
    ...
    # OHLCV is required; `oi` is an optional extra that Kite supplies per candle.
    failures += not check("candles normalized",
                          ["timestamp", "open", "high", "low", "close", "volume"]
                          == list(df.columns)[:6], str(list(df.columns)))
    ...
    # --- broker abstraction: Kite must satisfy the same contract ---
    from feed_base import MarketFeed as _MF
    import feed_kite as _fk
    for method in ("connect", "get_historical", "get_live_quote", "get_oi",
                   "get_expiries", "get_option_quote", "get_fno_stocks", "get_bulk_quotes"):
        failures += not check(f"KiteFeed implements {method}",
                              callable(getattr(_fk.KiteFeed, method, None)))
    failures += not check("KiteFeed owns a stream lifecycle",
                          all(callable(getattr(_fk.KiteFeed, m, None))
                              for m in ("start_stream", "stop_stream", "stream_healthy")))
    failures += not check("Kite token persistence implemented",
                          all(callable(getattr(_fk.KiteFeed, m, None))
                              for m in ("store_token", "load_token", "clear_token",
                                        "exchange_request_token", "login_url")))
    failures += not check("Kite interval mapping covers the scan timeframes",
                          _fk.INTERVALS.get("5m") == "5minute"
                          and _fk.INTERVALS.get("15m") == "15minute",
                          str({k: _fk.INTERVALS[k] for k in ("5m", "15m", "1d")}))
    failures += not check("KiteTickStream reuses the shared bar builder",
                          "BarBuilder" in pathlib.Path("feed_kite.py").read_text())

    # No path may lead back to the removed broker. The needle is assembled at
    # runtime so this check does not match its own source file.
    needle = "5" + "paisa"
    leftovers = [f.name for f in pathlib.Path(".").glob("*.py")
                 if f.name != "selftest.py" and needle in f.read_text().lower()]
    failures += not check("no removed-broker references remain", not leftovers,
                          ", ".join(leftovers) if leftovers else "clean")
    failures += not check("unknown FEED_MODE fails loudly instead of defaulting",
                          "raise ValueError" in pathlib.Path("feed_mock.py").read_text())

    # --- OI now comes from the candles, costing no extra call ---
    oi_df = await feed.get_historical("NIFTY 50", "5m", 200)
    failures += not check("candles carry open interest", "oi" in oi_df.columns)
    from scanner import IndexScanner as _IS
    oi_calc = _IS._oi_from_candles(oi_df)
    failures += not check("OI change derived from candles, no extra API call",
                          oi_calc is not None and oi_calc.get("source") == "candle",
                          f"{oi_calc['change_pct']:+.2f}%" if oi_calc else "none")
    ...
    # Every scheduled job must actually exist. A missing one raises inside the
    # FastAPI lifespan, uvicorn never binds, Railway's health check times out
    # and the deploy is rejected while the previous image keeps serving - so the
    # symptom is "nothing I upload takes effect", not a visible crash.
    import ast
    main_src = pathlib.Path("main.py").read_text()
    tree = ast.parse(main_src)
    defined = {n.name for n in ast.walk(tree)
               if isinstance(n, (ast.AsyncFunctionDef, ast.FunctionDef))}
    scheduled = {node.args[0].id for node in ast.walk(tree)
                 if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute)
                 and node.func.attr == "add_job" and node.args
                 and isinstance(node.args[0], ast.Name)}
    missing_jobs = sorted(scheduled - defined)
    failures += not check("every scheduled job is defined", not missing_jobs,
                          f"MISSING: {', '.join(missing_jobs)}" if missing_jobs
                          else f"{len(scheduled)} jobs, all defined")

    # The health check must not wait on the broker, or Railway rejects the deploy.
    lifespan_body = main_src.split("async def lifespan")[1][:700] if "async def lifespan" in main_src else ""
    failures += not check("startup does not block on the broker connection",
                          "create_task(warm_up())" in main_src
                          and "await connect_feed()" not in lifespan_body)
    ...
    # --- a bad DATABASE_URL must degrade, never crash-loop the whole app ---
    from database import _build_engine as _try_build_engine

    for label, bad_url in (
        ("unresolved Railway template", "${{Postgres.DATABASE_URL}}"),
        ("quoted paste artifact", '"postgresql://u:p@nonexistent.invalid:5432/db"'),
        ...
        try:
            ...
            failures += not check(f"bad DATABASE_URL ({label}) falls back instead of crashing",
                                  eng.dialect.name == "sqlite", f"got dialect={eng.dialect.name}")
    ...
    failures += not check("bid-ask spread captured", plan.spread is not None,
                          f"bid {plan.bid} / ask {plan.ask} = {plan.spread}")
    failures += not check("spread measured against the expected move, not premium",
                          plan.spread_drag_pct is not None
                          and plan.spread_drag_pct != plan.spread_pct,
                          f"{plan.spread_pct}% of premium but {plan.spread_drag_pct}% of the move")
    failures += not check("strike open interest captured", plan.open_interest is not None)

    # a deliberately illiquid contract must be blocked
    saved_cap = cfg.MAX_SPREAD_DRAG_PCT
    cfg.MAX_SPREAD_DRAG_PCT = 1.0
    tight = await layer.build_plan("NIFTY 50", 24500.0, "LONG", target_move=25.0)
    failures += not check("wide spread blocks the option leg",
                          tight.blocked and "spread" in (tight.block_reason or "").lower(),
                          tight.block_reason or "not blocked")
    cfg.MAX_SPREAD_DRAG_PCT = saved_cap
