...
class IndexScanner:
    ...
    async def _scan_symbol(self, symbol: str, timeframe: str) -> Optional[Dict]:
        ...
        if cfg.USE_OI_FILTER and self._worth_a_second_look(df):
            # Kite returns open interest inside the candles, so the change is a
            # real bar-over-bar figure rather than the difference between two
            # polled snapshots taken at arbitrary moments. Fall back to polling
            # only when the feed cannot supply it.
            oi = self._oi_from_candles(df)
            if oi is None:
    ...
    @staticmethod
    def _oi_from_candles(df: pd.DataFrame) -> Optional[Dict]:
        """Bar-over-bar OI change, when the feed embeds OI in the candles."""
        if "oi" not in df.columns or len(df) < 2:
            return None
        series = pd.to_numeric(df["oi"], errors="coerce").dropna()
        if len(series) < 2:
            return None
        current, previous = float(series.iloc[-1]), float(series.iloc[-2])
        if previous <= 0 or current <= 0:
            return None
        return {"oi": current, "prev_oi": previous,
