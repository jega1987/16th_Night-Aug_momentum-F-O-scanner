...
class CredentialsError(RuntimeError):
    """Configuration is wrong or incomplete. Retrying will not fix it."""
...
class FivePaisaFeed(MarketFeed):
    ...
    def __init__(self):
        ...
        self.resolver = None
        self.stream = None                 # TickStream once streaming starts

    # ------------------------------------------------------------------ #
    async def connect(self) -> bool:
        # Credentials first: it is the common failure and the cheapest to check.
        missing = cfg.missing_credentials()
        if missing:
            raise CredentialsError("Missing Railway variables: " + ", ".join(missing))

        # A missing library will not appear on a retry timer either.
        if FivePaisaClient is None:
            raise CredentialsError(
                "py5paisa is not installed in the image. The Docker build should have "
        ...
        def _login():
            ...
            # Path 1: a token you already hold. Skips the handshake entirely.
            if cfg.FIVEPAISA_ACCESS_TOKEN:
                client.access_token = cfg.FIVEPAISA_ACCESS_TOKEN
                client.Jwt_token = cfg.FIVEPAISA_ACCESS_TOKEN
                if hasattr(client, "_set_client_code"):
                    client._set_client_code(cfg.FIVEPAISA_CLIENT_CODE)
                else:
                    client.client_code = cfg.FIVEPAISA_CLIENT_CODE
                logger.info("[5paisa] Using FIVEPAISA_ACCESS_TOKEN (no TOTP handshake)")
                return client

            # Path 2: exchange a request token for an access token.
            if cfg.FIVEPAISA_REQUEST_TOKEN:
                client.get_access_token(cfg.FIVEPAISA_REQUEST_TOKEN)
                if not getattr(client, "access_token", ""):
                    raise CredentialsError(
                        "FIVEPAISA_REQUEST_TOKEN was rejected - request tokens are "
                        "single-use and expire quickly. Use FIVEPAISA_TOTP_SECRET instead.")
                return client

            # Path 3: the morning TOTP handshake (recommended - survives restarts).
            ...
            if not totp and cfg.FIVEPAISA_TOTP_SECRET:
                ...
                try:
                    ...
                    raise CredentialsError(
                        f"FIVEPAISA_TOTP_SECRET is not a valid base32 seed ({exc}). "
                        "Use the seed string from the 5paisa TOTP setup screen, "
                        "not the 6-digit code it displays.")

            client.get_totp_session(cfg.FIVEPAISA_CLIENT_CODE, totp, cfg.FIVEPAISA_PIN)
            token = getattr(client, "access_token", "")
            if not token:
                # py5paisa swallows its own auth exceptions and returns None, so
                # the specific reason is only in its log line above this one.
                raise CredentialsError(
                    "5paisa rejected the login. The usual causes, in order: the TOTP seed "
                    "is wrong or TOTP is not enabled on the account; the PIN is wrong; "
                    "the client code is not the login ID; or the API key is not active. "
                    "Check the py5paisa log line directly above for the exact message.")
            return client
    ...
    def start_stream(self, symbols: List[str]) -> bool:
        """
        Open the streaming connection after the morning handshake.

        One login, one socket, then ticks replace per-cycle polling. Returns
        False rather than raising, because a stream that won't start must
        degrade to REST rather than take the scanner down.
        """
        ...
        from feed_ws import TickStream

        scrip_map: Dict[str, Dict] = {}
        for symbol in symbols:
            try:
                scrip = self._scrip(symbol, use_futures=True)
                scrip_map[symbol] = {"Exch": scrip["Exch"], "ExchType": scrip["ExchType"],
                                     "ScripCode": int(scrip["ScripCode"])}
            except KeyError:
                logger.warning("[WS] No scrip mapping for %s - not streamed", symbol)
        ...
        self.stream = TickStream(self.client, scrip_map, minutes=minutes)
        self.stream.start()
        logger.info("[WS] Streaming %d instrument(s) at %dm bars", len(scrip_map), minutes)
        return True
