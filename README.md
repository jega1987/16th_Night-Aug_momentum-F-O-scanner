...
Runs on **Zerodha Kite Connect**. `FEED_MODE` accepts `kite` or `mock` and
nothing else — an unrecognised value raises at startup rather than falling back
to a default, because a silent fallback once had the app talking to one broker
while its own startup banner reported another.

Adding a different broker means writing one class against the `MarketFeed`
contract in `feed_base.py`. Nothing above that layer knows which broker is in
use.

### The daily token

Kite access tokens expire every morning and can only be minted through a browser
redirect, which does not fit an always-on container. So:

1. Register `https://<your-app>.up.railway.app/kite/callback` as the redirect URL
   on the Kite developer console.
2. Set `KITE_API_KEY` and `KITE_API_SECRET` in Railway.
3. Each morning, open `/kite/login` and sign in.

The token is exchanged server-side and persisted in the database, so it survives
restarts and redeploys — one click a day, no redeploy. A token issued yesterday
is treated as absent rather than sent to the API, so you get "please log in"
instead of a confusing session error. `/kite/status` reports whether one is held.

`/kite/callback` is deliberately not behind `DASHBOARD_TOKEN`: Kite controls the
redirect and cannot append a custom query parameter. The `request_token` it
carries is single-use, short-lived, and worthless without the API secret, which
never leaves the server.

### What Kite gives you

- **Open interest arrives inside the candles** (`oi=True`), so OI change is a
  real bar-over-bar series and costs no extra API call.
- **Ticks carry OI**, so the streamed path keeps it.
- **Quotes carry market depth**, which enables the option bid-ask spread gate.
  Note that spread is measured against the expected move, not against premium:
  a spread worth 1.4% of premium can be 14% of what the trade expects to make.
- **`KiteTicker.connect(threaded=True)`** manages its own thread and
  reconnection, so there is no hand-rolled socket loop.

Kite's rate limits are roughly 3/s historical and 1/s quote, so
`API_CALLS_PER_SECOND` defaults conservatively. Historical data is a
separate paid add-on to the Kite Connect subscription; the candle store and
backfill both depend on it.
...
**`Feed not configured`, listing missing variables.** Set `KITE_API_KEY` and
`KITE_API_SECRET`, then sign in at `/kite/login`. Every missing variable is
listed in one log block and on `/health` under `config.missing`. Retries are
suspended once a configuration error is detected, because a missing variable
will still be missing in five minutes.

**`/kite/status` returns `Not Found`.** The running image predates the Kite
build. Confirm with Railway's Console: `ls /app/feed_kite.py`. If the file is
...
rank needs weeks of history to become usable.
...
feed_kite.py       Kite Connect: REST, instruments, options, KiteTicker
feed_ws.py         broker-agnostic tick aggregation and REST splice
