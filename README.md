...
**`Feed connection failed: Set FIVEPAISA_TOTP_SECRET`, repeating.** The feed
credentials are missing. The app is idle, not broken — the web server, database
and scheduler are all fine. Every missing variable is listed in one log block
and on `/health` under `config.missing`. Retries are suspended once a
configuration error is detected, because a missing variable will still be
missing in five minutes; fix the variables and redeploy.

Three ways to authenticate, in order of preference:

| Variables | When to use |
|---|---|
| `FIVEPAISA_TOTP_SECRET` + `FIVEPAISA_PIN` | Recommended. The base32 seed from the 5paisa TOTP setup screen — **not** the 6-digit code it displays. A fresh code is generated at every login, so sessions survive restarts. |
| `FIVEPAISA_ACCESS_TOKEN` | You already hold a day token. Skips the handshake entirely; expires daily. |
| `FIVEPAISA_REQUEST_TOKEN` | Exchanged for an access token. Single-use and short-lived. |

All three also need `FIVEPAISA_USER_KEY`, `FIVEPAISA_ENCRYPTION_KEY` and
`FIVEPAISA_CLIENT_CODE`.
