...
class StrategyConfig:
    ...
    # ---------- infra ----------
    FEED_MODE: str = os.getenv("FEED_MODE", "5paisa").strip().lower()   # "5paisa" | "mock"
    ...
    # ---------- 5paisa credentials ----------
    FIVEPAISA_APP_NAME: str = os.getenv("FIVEPAISA_APP_NAME", "")
    FIVEPAISA_APP_SOURCE: str = os.getenv("FIVEPAISA_APP_SOURCE", "")
    FIVEPAISA_USER_ID: str = os.getenv("FIVEPAISA_USER_ID", "")
    FIVEPAISA_PASSWORD: str = os.getenv("FIVEPAISA_PASSWORD", "")
    FIVEPAISA_USER_KEY: str = os.getenv("FIVEPAISA_USER_KEY", "")
    FIVEPAISA_ENCRYPTION_KEY: str = os.getenv("FIVEPAISA_ENCRYPTION_KEY", "")
    FIVEPAISA_CLIENT_CODE: str = os.getenv("FIVEPAISA_CLIENT_CODE", "")
    FIVEPAISA_PIN: str = os.getenv("FIVEPAISA_PIN", "")
    FIVEPAISA_TOTP_SECRET: str = os.getenv("FIVEPAISA_TOTP_SECRET", "")  # base32 seed
    FIVEPAISA_TOTP: str = os.getenv("FIVEPAISA_TOTP", "")                # or a one-shot code
    # Skip the TOTP handshake entirely if you already hold a day token.
    FIVEPAISA_ACCESS_TOKEN: str = os.getenv("FIVEPAISA_ACCESS_TOKEN", "")
    FIVEPAISA_REQUEST_TOKEN: str = os.getenv("FIVEPAISA_REQUEST_TOKEN", "")
    ...
    def missing_credentials(self) -> List[str]:
        ...
        Railway that is a slow way to find out you also forgot the PIN.
        """
        if self.FEED_MODE != "5paisa":
            return []

        missing = [name for name, value in (
            ("FIVEPAISA_USER_KEY", self.FIVEPAISA_USER_KEY),
            ("FIVEPAISA_ENCRYPTION_KEY", self.FIVEPAISA_ENCRYPTION_KEY),
            ("FIVEPAISA_CLIENT_CODE", self.FIVEPAISA_CLIENT_CODE),
        ) if not value]

        # Auth path: a pre-issued token, or the TOTP handshake.
        if not self.FIVEPAISA_ACCESS_TOKEN and not self.FIVEPAISA_REQUEST_TOKEN:
            if not (self.FIVEPAISA_TOTP_SECRET or self.FIVEPAISA_TOTP):
                missing.append("FIVEPAISA_TOTP_SECRET (or FIVEPAISA_TOTP / FIVEPAISA_ACCESS_TOKEN)")
            if not self.FIVEPAISA_PIN:
                missing.append("FIVEPAISA_PIN")
        return missing
