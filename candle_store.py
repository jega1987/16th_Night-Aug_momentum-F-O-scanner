...
* It reduces *payload*, not *call count*. The historical endpoint takes a date
  range, so the smallest possible request is still one day - one call per symbol
  per cycle either way. Fetching 75 bars instead of 200+ across 7 days is a big
  bandwidth saving and a small rate-limit saving.
...
class CandleStore:
    ...
    async def get(self, symbol: str, timeframe: str, bars: int,
        ...
        """
        Best available series: stored history, the REST gap if needed, and live
        streamed bars spliced on top.

        When the stream is healthy it supplies the current session, so the REST
        call is skipped entirely - that is where the token saving actually
        comes from. A stale stream falls back to REST rather than scanning
        frozen data.
        """
        live = self._live_bars(symbol, timeframe)
        stored = self.read(symbol, timeframe, bars)
        last_ts = stored["timestamp"].iloc[-1] if len(stored) else None

        have_history = len(stored) >= bars * 0.9
        if live is not None and not live.empty and have_history:
            self.api_calls_saved += 1
            return splice(stored, live).tail(bars).reset_index(drop=True)

        if not self._needs_refresh(last_ts, timeframe):
            self.api_calls_saved += 1
            logger.debug("[Store] %s %s served from cache (%d bars)", symbol, timeframe, len(stored))
            base = stored.tail(bars).reset_index(drop=True)
            return splice(base, live) if live is not None and not live.empty else base
        ...
        base = merged if len(merged) else fresh
        return splice(base, live).tail(bars).reset_index(drop=True) \
            if live is not None and not live.empty else base

    def _live_bars(self, symbol: str, timeframe: str) -> Optional[pd.DataFrame]:
        """
        Completed bars are persisted; the forming bar is returned but never
        written, or a half-built candle would be cached and reused as final.
        """
        stream = getattr(self.feed, "stream", None)
        if not (cfg.USE_WEBSOCKET and stream and stream.connected):
            return None
        if stream.is_stale():
            logger.warning("[Store] Stream is stale - falling back to REST for %s", symbol)
            return None
        if timeframe != cfg.TIMEFRAME:
            return None            # the stream only builds the scan timeframe

        builder = stream.builders.get(symbol)
        if builder is None:
            return None
        completed = builder.frame(include_forming=False)
        if not completed.empty:
            self.write(symbol, timeframe, completed)
        frame = builder.frame(include_forming=True)
