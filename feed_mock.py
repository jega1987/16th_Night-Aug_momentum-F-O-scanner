...
def build_feed() -> MarketFeed:
    """Factory used by the app. FEED_MODE=mock skips the broker entirely."""
    mode = cfg.FEED_MODE
    if mode == "mock":
        return MockFeed()
    if mode == "kite":
        ...
        return KiteFeed()
    if mode == "5paisa":
        from feed_5paisa import FivePaisaFeed
        return FivePaisaFeed()
    raise ValueError(f"Unknown FEED_MODE '{mode}'. Use kite, 5paisa or mock.")
